...
const tableData = atom((get) =>
    get(currentCompany)
        .tables.map(
            (tableID) => get(allTables).find((table) => table.id === tableID)!
        )
        .map((table): ChangingTableResult => {
            const resultOld = findResult(
                table,
                get(filters).find((filter) => filter.tableID === table.id)
                    ?.tableFilters!
            );
            const resultNew = findResult(
                table,
                get(savedUrlFilters).find(
                    (filter) => filter.tableID === table.id
                )?.tableFilters!
            );

            const haveUpdates = resultNew.filtered !== resultOld.filtered;
            const switchStatus = haveUpdates
                ? tableSwitchStatus.changed
                : tableSwitchStatus.notChanged;
...
const SwitcherTables = () => {
    ...
    const tdChangeable = (
        ...
        const firstDisplayedNumber =
            mouseHoverVersion === selectedType.new ? newData : oldData;
    ...
    return (
        <div className="tableDiv">
            <table className="HiringCampaignsCompare">
                <thead>
                    <tr>
